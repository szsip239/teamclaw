...
- 只在空值或占位符时生成 JWT / 加密密钥，复跑时保留已有密钥
...
# 编辑 .env：TEAMCLAW_DATA_DIR、RAG_SERVICE_SECRET、可选 Nginx 证书配置
...
| `RAG_SERVICE_SECRET`                                    | Next.js 与 RAG 服务之间的内部鉴权密钥，两边必须一致                             |
...
- Generate JWT/encryption secrets only when values are missing or placeholders; reruns preserve existing secrets
...
cp .env.example .env
node scripts/generate-keys.mjs --write
# Edit .env: TEAMCLAW_DATA_DIR, RAG_SERVICE_SECRET, optional Nginx settings
...
| `RAG_SERVICE_SECRET`                                    | Internal shared secret between Next.js and the RAG service                                                   |
